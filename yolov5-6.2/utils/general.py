...
def try_except(func):
    """Decorator: catch and log exceptions with a warning, returns None on failure."""
...
def clip_coords(boxes, shape):
    ...
    if boxes is None:
        return
    h, w = (shape[0], shape[1]) if isinstance(shape, (list, tuple)) else shape[:2]
    if hasattr(boxes, "clamp_"):  # torch.Tensor
        boxes[:, 0].clamp_(0, w)
        boxes[:, 1].clamp_(0, h)
        boxes[:, 2].clamp_(0, w)
        boxes[:, 3].clamp_(0, h)
