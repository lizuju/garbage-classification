...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, prob in enumerate(pred):  # per image
            ...
            top5 = [f"{map_index_to_chinese(j, model_names, cn_names, dict_order)} {prob[j]:.2f}" for j in top5i]
            top1_idx = top5i[0]
            top1_label = map_index_to_chinese(top1_idx, model_names, cn_names, dict_order)
            s += f"Top-5: {', '.join(top5)} | Top-1: {top1_label} {prob[top1_idx]:.2f}, "

            # Write results
            text = "\n".join(f"{prob[j]:.2f} {map_index_to_chinese(j, model_names, cn_names, dict_order)}" for j in top5i)
