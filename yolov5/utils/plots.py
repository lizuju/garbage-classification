...
def plot_results(file="path/to/results.csv", dir=""):
    ...
    for f in files:
        try:
            ...
            indices = [1, 2, 3, 4, 5, 8, 9, 10, 6, 7] if n > 10 else list(range(1, n))
            for i, j in enumerate(indices):
                if i >= len(ax):
                    break
                y = data.values[:, j].astype("float")
                if j > 1:  # Only for validation/metrics, not for epoch or train_loss (which could be near-zero but actual)
                    y[y == 0] = np.nan  # don't show zero values for intermittent validation
