...
def reshape_classifier_output(model, n=1000):
    ...
    if isinstance(m, Classify):  # YOLOv5 Classify() head
        if hasattr(model, "nc_major") and getattr(model, "nc_major") is not None:
            from models.common import HierarchicalClassify
            if not isinstance(m, HierarchicalClassify):
                m.__class__ = HierarchicalClassify
                m.linear_sub = nn.Linear(m.linear.in_features, n)
                m.linear_major = nn.Linear(m.linear.in_features, getattr(model, "nc_major"))
                del m.linear
        elif m.linear.out_features != n:
