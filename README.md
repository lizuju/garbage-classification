...
- **265 类细粒度 + 4 大类联合优化**：模型同时输出细分类别与主类预测，单次推理即可获得精细结果与大类归属，无需二次分类。
- **层级损失平衡（`major_weight=0.5`）**：主类损失与细分类损失以相同权重联合回传，防止模型过度偏向细粒度而忽视大类语义，大类预测鲁棒性显著提升。
- **标签平滑抑制过拟合**：`label_smoothing=0.1` 使模型输出概率更为平滑，减少对训练集噪声标注的记忆，泛化能力更强。
- **高分辨率输入提升细粒度区分**：采用 448×448 输入（优于标准 224），更完整保留外观细节，对外形相似垃圾（如不同类型塑料瓶）区分效果更佳。
- **ImageNet 迁移 + Adam 微调**：基于 YOLOv5x-cls 预训练权重，`lr0=0.001` + Adam 优化器精细微调，收敛更快且精度更高。
- **FP16 半精度 GPU 加速**：生产推理自动启用 FP16，速度提升约 1.5~2×，显存占用减半。
- **帧平均稳定实时识别**：摄像头模式下对连续 3 帧概率取均值，彻底消除单帧抖动，识别结果更稳定。
- **权重热替换支持持续迭代**：新权重放置指定路径后重启即生效，无需修改任何配置，支持模型持续优化与快速上线。
